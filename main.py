...
async def receive_murojaat(message: types.Message, state: FSMContext):
    ...
    # --- ADMIN CALLBACKLARI, REKLAMA, REF BONUS VA TO'LOV HANDLERLARI ---

# Admin panelining handlerlari
...
async def edit_balance_prompt(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Foydalanuvchi hisobini qancha qilmoqchisiz? Son kiritib yuboring:")
...
async def process_new_balance(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ Foydalanuvchi (ID: <code>{target_user_id}</code>) hisobi <b>{new_balance}</b> olmos qilib belgilandi!", parse_mode="HTML")

    try:
        await bot.send_message(
            target_user_id,
            f"🔔 Adminga ko'ra hisobingiz o'zgartirildi! Hozirgi balansingiz: <b>{new_balance} 💎</b>",
            parse_mode="HTML"
        )
    except Exception:
        pass

# --- REF BONUS O'ZGARTIRISH ---
@dp.message(F.text == "Ref bonus🔗")
async def ask_ref_bonus(message: types.Message, state: FSMContext):
    if message.from_user.id == ADMIN_ID:
        await state.set_state(AdminPanelState.waiting_for_ref_bonus)
        await message.answer("Referal uchun yangi bonus miqdorini yozib yuboring ❗💎")

@dp.message(AdminPanelState.waiting_for_ref_bonus)
async def process_ref_bonus(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return

    if not message.text.isdigit():
        await message.answer("Iltimos, faqat butun son yozib yuboring ❗")
        return

    new_bonus = message.text.strip()
    set_setting("ref_bonus", new_bonus)
    await state.clear()
    await message.answer(f"✅ Referal bonusi muvaffaqiyatli <b>{new_bonus} 💎</b> qilib belgilandi!", parse_mode="HTML")
...
async def ask_broadcast_text(message: types.Message, state: FSMContext):
    if message.from_user.id == ADMIN_ID:
        ...
        await message.answer("Reklama matnini yozib yuboring ❗")
...
async def process_broadcast(message: types.Message, state: FSMContext):
    ...
    await message.answer("Barcha foydalanuvchiga yuborildi ✅")

# --- TO'LOV SO'ROVINI TASDIQLASH (QO'LDA) ---
...
async def approve_withdrawal(call: types.CallbackQuery):
    ...
    await call.message.edit_text(
        call.message.text + "\n\n✅ <b>TASDIQLANDI</b> (Olmos o'tkazib berildi)",
        parse_mode="HTML"
    )
    try:
        await bot.send_message(
            user_id,
            f"✅ So'rovingiz tasdiqlandi! Yumicoin hisobingizga <b>{amount} olmos</b> o'tkazib berildi!",
            parse_mode="HTML"
        )
    except Exception:
        pass
...
async def reject_withdrawal(call: types.CallbackQuery):
    ...
    await call.message.edit_text(
        call.message.text + "\n\n❌ <b>RAD ETILDI</b> (Olmos qaytarib berildi)",
        parse_mode="HTML"
    )
    try:
        await bot.send_message(
            user_id,
            f"❌ So'rovingiz rad etildi! <b>{amount} olmos</b> balansizga qaytarildi.",
            parse_mode="HTML"
        )
    except Exception:
        pass
...
# --- ADMINDAN MUROJAATGA JAVOB BERISH (REPLY ORQALI) ---
...
# --- WEB SERVER (Render 24/7) VA BOTNI ISHGA TUSHIRISH ---
async def handle(request):
    return web.Response(text="Bot 24/7 rejimida muvaffaqiyatli ishlayapti!")
